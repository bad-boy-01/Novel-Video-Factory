...
class LocalImageAdapter:
    ...
    def _init_pipeline(self):
        ...
        try:
            ...
            # This is commented out to prevent massive background downloads during tests
            # self.pipeline = AutoPipelineForText2Image.from_pretrained(
            #     self.model_id, torch_dtype=torch.float16
            # )
            # self.pipeline.enable_model_cpu_offload()
            logger.info(f"Initialized Diffusers pipeline for {self.model_id} (MOCK MODE ENABLED for testing)")
        except ImportError:
